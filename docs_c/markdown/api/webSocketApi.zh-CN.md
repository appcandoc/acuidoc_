...
<ac-view class="content">
  <ac-button bindtap="establish">创建 WebSocket 连接</ac-button>
  <ac-button bindtap="close">关闭 WebSocket 连接</ac-button>
  <ac-input value="{{exampleText}}" bindinput="handleInputChange" placeholder="请输入文字测试" disabled="{{!open}}"></ac-input>
  <ac-button bindtap='sendSocketMessage' type="primary" disabled="{{!disabled}}">通过 WebSocket 连接发送数据</ac-button>
...
```javascript
const sendUrl = 'ws://www.uileader.com/touchac-backstage/websocket.do'
export default {
  data () {
    return {
      loading: false,
